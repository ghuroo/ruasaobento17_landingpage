var keystone = require('keystone'),
	async = require('async'),
	HomeHighlights = keystone.list('HomeHighlights'),
...
function createItem(item, done) {
    Language.model.findOne({ iso: item.language }).exec(function(err, result) {
        ...
        HomeHighlights.model.findOne({ name: item.name }).exec(function(err, result) {
            if (err || result) return done();

            new HomeHighlights.model(item).save(function(err) {
